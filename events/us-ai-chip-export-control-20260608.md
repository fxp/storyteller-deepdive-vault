---
id: us-ai-chip-export-control-20260608
title: 美国 AI 芯片出口管制执法升级（追踪技术 / 走私执法 / 逐案审查）
seed_url: https://www.morganlewis.com/pubs/2026/01/bis-revises-export-review-policy-for-advanced-ai-chips-destined-for-china-and-macau
published_date: 2026-01-15
status: ACTIVE
last_check: 2026-06-09T09:04
last_activity: '2026-06-08'
next_check: 2026-06-09T15:04
interval_hours: 6
consecutive_empty: 5
findings_count: 0
tags:
- tracker
- ACTIVE
entities:
  orgs:
  - BIS（商务部工业与安全局）
  - NVIDIA
  - AMD
  - Super Micro Computer
  - 美国国会
  - 美国司法部
  people:
  - 吉娜·雷蒙多（前商务部长）
  - Howard Lutnick（现商务部长）
  products:
  - NVIDIA H200
  - NVIDIA B200（Blackwell）
  - AMD MI325X
  - NVIDIA H100
  topics:
  - 芯片出口管制
  - 《芯片安全法》
  - 走私执法
  - 逐案审查
  - 中美科技脱钩
search_queries:
- BIS AI chip export license case-by-case review China 2026
- Chip Security Act tracking technology NVIDIA implementation
- AI chip smuggling arrest enforcement 2026 Super Micro
- Blackwell B200 export control China restriction 2026
- NVIDIA AMD China AI chip sales approval denial 2026
---

# 美国 AI 芯片出口管制执法升级（追踪技术 / 走私执法 / 逐案审查）

> 追踪卡片 · 状态: HOT · 下次检查: 2026-06-08T15:00

## 种子事件

**日期**: 2026-01-15
**来源**: [BIS Revises Export Review Policy for Advanced AI Chips — Morgan Lewis](https://www.morganlewis.com/pubs/2026/01/bis-revises-export-review-policy-for-advanced-ai-chips-destined-for-china-and-macau)

**摘要**: 2026 年 1 月，BIS 将 H200/MI325X 对华出口政策从"推定拒绝"改为"逐案审查"，同期《芯片安全法》获国会批准，拟在出口芯片中植入追踪技术（Kill Switch）。3 月，超微电脑联合创始人因涉嫌组织 AI 芯片走私网络被捕，成为该法律体系下首个重大执法案例。管制执法与规避手段（经第三国转运）的博弈持续升级。

## 关键实体

- **组织**: BIS、NVIDIA、AMD、Super Micro Computer、美国司法部
- **人物**: Howard Lutnick（商务部长）
- **产品**: H200、B200（Blackwell）、MI325X、H100
- **赛道**: 半导体出口管制 / 芯片追踪技术 / 走私执法 / 逐案审查机制

## 预期后续方向

1. 《芯片安全法》实施细则出台——追踪技术技术规格公开
2. BIS 对 Blackwell（B200 以上）是否纳入管控范围的决定
3. 更多走私案件曝光（经中东/东南亚的转运网络）
4. 中国对管制升级的反制措施（矿产 / 材料 / 软件）

## 后续发现

<!-- 监控代理自动追加 -->

## 相关文章

- [Al Jazeera: US says ban on AI chip shipments applies to Chinese firms outside China](https://www.aljazeera.com/economy/2026/6/1/us-says-ban-on-ai-chip-shipments-applies-to-chinese-firms-outside-china)
- [BISI: AI chip smuggling — The Limits of US Export Controls](https://bisi.org.uk/reports/ai-chip-smuggling-the-limits-of-us-export-controls)
